...
function isValid(x: number, nums: number[]) {
    ...
    if (nums.length == 2) {
        ...
        if (x === nums[0] + nums[1]) return true
        if (x === parseInt(String(nums[0]) + String(nums[1]))) return true
        return false
    }

    return isValid(x, [nums[0] * nums[1], ...nums.slice(2)]) || isValid(x, [nums[0] + nums[1], ...nums.slice(2)]) || isValid(x, [parseInt(String(nums[0]) + String(nums[1])), ...nums.slice(2)])
}

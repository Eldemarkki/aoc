...
const tree = parsePacket(Array.from(bitsStr).map(Number));

const eval = tree => {
  if (!tree.subPackets) {
    return tree.decimalValue
  }

  if (tree.packetTypeId === 0) {
    return tree.subPackets.reduce((prev, curr) => prev + eval(curr), 0)
  }
  else if (tree.packetTypeId === 1) {
    if (tree.subPackets.length === 1) return eval(tree.subPackets[0])
    return tree.subPackets.reduce((prev, curr) => {
      return prev * eval(curr)
    }, 1)
  }
  else if (tree.packetTypeId === 2) {
    return Math.min(...tree.subPackets.map(p => eval(p)))
  }
  else if (tree.packetTypeId === 3) {
    return Math.max(...tree.subPackets.map(p => eval(p)))
  }
  else if (tree.packetTypeId === 5) {
    // Has exactly 2 sub-packets
    const a = eval(tree.subPackets[0])
    const b = eval(tree.subPackets[1])
    return a > b ? 1 : 0
  }
  else if (tree.packetTypeId === 6) {
    // Has exactly 2 sub-packets
    const a = eval(tree.subPackets[0])
    const b = eval(tree.subPackets[1])
    return a < b ? 1 : 0
  }
  else if (tree.packetTypeId === 7) {
    // Has exactly 2 sub-packets
    const a = eval(tree.subPackets[0])
    const b = eval(tree.subPackets[1])
    return a === b ? 1 : 0

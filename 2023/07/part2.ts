...
const getType = (cards: Card[]) => {
  ...
  const firstCount = sorted.filter((x) => x === first).length;
  let map = new Map<Card, number>();
  cards.forEach((c) => map.set(c, (map.get(c) ?? 0) + 1));

  const most = [...map.entries()].filter((x) => x[0] !== "J");
  most.sort((a, b) => b[1] - a[1]);
  let treatJokersAs = "A" as Card;
  treatJokersAs = most.length ? most[0][0] : cards[0];

  cards = cards.map((c) => (c === "J" ? treatJokersAs : c));
  map = new Map<Card, number>();
  cards.forEach((c) => map.set(c, (map.get(c) ?? 0) + 1));
